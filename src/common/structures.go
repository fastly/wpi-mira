...
type Result struct {
	...
	AllFreq     map[time.Time]int `json:"AllFreq,omitempty"`     //map of all the frequencies by timestamp to avoid repeats and keep track of missed data
}
...
type OutlierInfo struct {
	...
	Count     int       `json:"Count,omitempty" `    //the number of messages in the bucket that is an outlier
